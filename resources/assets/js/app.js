
/**
 * First we will load all of this project's JavaScript dependencies which
 * includes Vue and other libraries. It is a great starting point when
 * building robust, powerful web applications using Vue and Laravel.
 */

require('./bootstrap');
require('admin-lte');
require('admin-lte/bower_components/raphael/raphael.js');
require('admin-lte/bower_components/morris.js/morris.css');
require('admin-lte/bower_components/jquery-sparkline/dist/jquery.sparkline.js');
require('admin-lte/bower_components/jquery-knob/dist/jquery.knob.min.js');
require('admin-lte/bower_components/moment/moment.js');
require('admin-lte/bower_components/bootstrap-daterangepicker/daterangepicker.js');
require('admin-lte/bower_components/bootstrap-datepicker/dist/js/bootstrap-datepicker.js');
require('admin-lte/bower_components/jquery-slimscroll/jquery.slimscroll.min.js');
require('admin-lte/bower_components/fastclick/lib/fastclick.js');
require('admin-lte/bower_components/datatables.net/js/jquery.dataTables.min.js');
require('admin-lte/bower_components/datatables.net-bs/js/dataTables.bootstrap.min.js');

